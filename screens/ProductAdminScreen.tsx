...
const ProductAdminScreen: React.FC = () => {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    if (isEditMode && currentProduct.id) {
      ...
      addProduct(productData as Product); // For new product, ID is generated by addProduct
    }
    closeModal();
  };
  ...
  const handleExport = async (format: 'csv' | 'xlsx' | 'pdf') => {
    ...
    // Prepare data, ensuring all optional fields are handled for consistency
  ...
  const handleImportChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    Papa.parse(file, {
      ...
      complete: (results) => {
        try {
          const importedProducts: Partial<Product>[] = results.data.map((row: any): Partial<Product> => {
            ...
            const rawQuantityType = row.quantityType;
            const finalQuantityType: 'unit' | 'kg' =
              typeof rawQuantityType === 'string' && rawQuantityType.toLowerCase() === 'kg'
                ? 'kg'
                : 'unit'; // Default to 'unit' if not 'kg' or if undefined/invalid
          ...
          }).filter((p): p is Partial<Product> & { name: string } => !!p.name && p.name.trim() !== '');
          ...
          if (importedProducts.length > 0) {
            ...
            setImportError(t('productAdmin.import.error', { error: "No valid products found in CSV or CSV is empty."}));
          }
        } catch (err: any) {
          ...
          <div className="bg-white p-6 sm:p-8 rounded-lg shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <input
                  id="imageUrl" name="imageUrlFile" type="file" // Name changed to avoid conflict if `currentProduct.imageUrl` is text
                  accept="image/*"
